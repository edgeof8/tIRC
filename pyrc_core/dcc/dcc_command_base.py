...
class DCCCommandHandlerBase:
    ...
    async def _handle_dcc_error(self, message: str, context_name: str, log_level: int = logging.ERROR, exc_info: bool = False):
        ...
        if TYPE_CHECKING:
            client_logic_cast = cast("IRCClient_Logic", self.client_logic)
            await client_logic_cast.add_message(message, client_logic_cast.ui.colors["error"], context_name=context_name)
        else:
