...
async def handle_dcc_cancel_command(client_logic: 'IRCClient_Logic', cmd_args: List[str], active_context_name: str, dcc_context_name: str):
    """
    ...
    if ambiguous_transfer:
        await _handle_dcc_error(client_logic, f"Ambiguous transfer ID prefix '{identifier_prefix}'. Multiple active transfers match.", dcc_context_name)
        return

    if actual_transfer_id_to_cancel:
        if dcc_m.cancel_transfer(actual_transfer_id_to_cancel):
            await client_logic.add_message(f"DCC transfer {actual_transfer_id_to_cancel[:8]} cancellation requested.", client_logic.ui.colors["system"], context_name=dcc_context_name)
            cancelled = True
    ...
    if not cancelled:
        await _handle_dcc_error(client_logic, f"No active transfer or pending passive offer found matching ID/token prefix '{identifier_prefix}'.", dcc_context_name)
