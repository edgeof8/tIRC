...
class ScriptManager:
    def __init__(
        ...
        self.registered_help_texts: Dict[str, Dict[str, Any]] = {} # For api.register_help_text
        ...
        try:
            ...
            for section in config.sections():
                ...
                for command, help_text in config[section].items():
                    self.ini_help_texts[section][command.lower()] = help_text # Store command lowercase
            self.logger.info(f"Successfully loaded help texts from '{help_ini_full_path}'")
        except Exception as e:
            self.logger.error(
                f"Error loading help texts from '{help_ini_full_path}': {e}", exc_info=True
            )
    ...
    def get_data_file_path_for_script(
        ...
        data_dir = os.path.join(self.base_dir, "data", script_name) # Store data in <project_root>/data/<script_name>/
                    ...
                    if hasattr(script_module, "get_script_instance"):
                        ...
                        if script_instance:
                            ...
                            if hasattr(script_instance, "load") and callable(script_instance.load):
                                ...
                                self.logger.info(f"Successfully loaded and initialized script: {script_name}")
                            else:
                                self.logger.debug(f"Script {script_name} instance created, but no 'load' method found.")
                    else:
                        self.logger.warning(f"Script {script_name} has no get_script_instance function")
                except Exception as e:
                    self.logger.error(f"Failed to load script {script_name}: {e}", exc_info=True) # Log full traceback
    ...
    def register_command_from_script(
        self, command_name: str, handler: Callable, help_text: str,
        aliases: Optional[List[str]] = None, script_name: Optional[str] = None,
    ) -> None:
        if aliases is None: aliases = []
        if script_name is None: script_name = "UnknownScriptFromRegisterCommand" # Fallback
        ...
        self.logger.info(f"Script '{script_name}' registered command: /{cmd_name_lower}")

    def register_help_text_from_script(
        self, command_name: str, help_text: str,
        aliases: Optional[List[str]] = None, script_name: Optional[str] = None
    ) -> None:
        if aliases is None: aliases = []
        if script_name is None: script_name = "UnknownScriptFromRegisterHelp"
        ...
        self.logger.info(f"Script '{script_name}' registered explicit help text for command: /{cmd_name_lower}")
        for alias in aliases:
            alias_lower = alias.lower()
            self.registered_help_texts[alias_lower] = {
                "help_text": help_text,
                "aliases": [cmd_name_lower] + [a.lower() for a in aliases if a.lower() != alias_lower],
                "script_name": script_name,
                "is_alias": True,
    ...
    def get_help_text_for_command(self, command_name: str) -> Optional[Dict[str, Any]]:
        ...
        if cmd_lower in self.registered_commands:
            ...
            return {
                ...
                "script_name": cmd_data.get("script_name", "script"), # Return the script name
        ...
        if cmd_lower in self.command_aliases:
            ...
            if primary_cmd_name in self.registered_commands:
                ...
                alias_list_for_this = [primary_cmd_name] + [a for a in all_aliases if a != cmd_lower]
                return {
                    "help_text": cmd_data.get("help_text", "No description available."),
        ...
        for section_name, section_content in self.ini_help_texts.items(): # Iterate through dict items
            if cmd_lower in section_content:
                return {"help_text": section_content[cmd_lower], "aliases": [], "script_name": "core_ini", "is_alias": False}
        return None
        ...
        for section_name, section_content in self.ini_help_texts.items():
            for cmd, text in section_content.items():
                ...
                if cmd not in all_help:
                    all_help[cmd] = {"help_text": text, "script_name": "core_ini", "aliases": []}
    ...
    def subscribe_script_to_event(
        ...
        if not callable(handler_function):
            self.logger.error(f"Script '{script_name}' attempted to subscribe non-callable handler for event '{event_name}'.")
            return
        if event_name not in self.event_subscriptions: self.event_subscriptions[event_name] = []
        for sub in self.event_subscriptions[event_name]:
            if sub["handler"] == handler_function and sub["script_name"] == script_name:
                self.logger.warning(f"Script '{script_name}' handler already subscribed to event '{event_name}'. Ignoring duplicate."); return
        self.event_subscriptions[event_name].append({"handler": handler_function, "script_name": script_name, "enabled": True})
        self.logger.info(f"Script '{script_name}' subscribed to event '{event_name}' with handler '{handler_function.__name__}'.")
    ...
    def unsubscribe_script_from_event(
        ...
        if event_name in self.event_subscriptions:
            self.event_subscriptions[event_name] = [
                sub for sub in self.event_subscriptions[event_name]
                if not (sub["handler"] == handler_function and sub["script_name"] == script_name)
            ]
            self.logger.info(f"Script '{script_name}' unsubscribed handler '{handler_function.__name__}' from event '{event_name}'.")
    ...
    def dispatch_event(
        ...
        if "raw_line" not in event_data: event_data["raw_line"] = ""
        ...
        if event_name in self.event_subscriptions:
            for subscription in list(self.event_subscriptions[event_name]):
                ...
                handler = subscription["handler"]; script_name = subscription["script_name"]
                try:
                    self.logger.debug(f"Calling handler '{handler.__name__}' from script '{script_name}' for event '{event_name}'.")
                    handler(event_data)
                except Exception as e:
                    self.logger.error(f"Error in event handler '{handler.__name__}' from script '{script_name}' for event '{event_name}': {e}", exc_info=True)
                    error_message = f"Error in script '{script_name}' event handler for '{event_name}': {e}"
                    self.client_logic_ref.add_message(error_message, self.client_logic_ref.ui.colors.get("error", 0), context_name="Status")
                    subscription["enabled"] = False
                    self.logger.warning(f"Disabled event handler '{handler.__name__}' from script '{script_name}' due to error.")
        else:
            self.logger.debug(f"No subscriptions found for event '{event_name}'.")

    def get_random_quit_message_from_scripts(self, variables: Dict[str, str]) -> Optional[str]:
        for script_name, instance in self.scripts.items():
            if hasattr(instance, "get_quit_message") and callable(instance.get_quit_message):
                try:
                    message_obj = instance.get_quit_message(variables)
                    if message_obj is not None:
                        if isinstance(message_obj, str): return message_obj
                        else: self.logger.warning(f"Script '{script_name}' get_quit_message returned non-string type. Ignoring.")
                except Exception as e: self.logger.error(f"Error calling get_quit_message on script '{script_name}': {e}", exc_info=True)
        return None

    def get_random_part_message_from_scripts(self, variables: Dict[str, str]) -> Optional[str]:
        for script_name, instance in self.scripts.items():
            if hasattr(instance, "get_part_message") and callable(instance.get_part_message):
                try:
                    message_obj = instance.get_part_message(variables)
                    if message_obj is not None:
                        if isinstance(message_obj, str): return message_obj
                        else: self.logger.warning(f"Script '{script_name}' get_part_message returned non-string type. Ignoring.")
                except Exception as e: self.logger.error(f"Error calling get_part_message on script '{script_name}': {e}", exc_info=True)
        return None
            ...
            try:
                ...
                if hasattr(module, "get_script_instance"):
                    ...
                    if script_instance:
                        ...
                        return True
                else: self.logger.warning(f"Script {script_name} has no 'get_script_instance' method")
            except Exception as e: self.logger.error(f"Failed to enable script {script_name}: {e}")
        return False
    ...
    def reload_script(self, script_name: str) -> bool:
        if script_name in self.scripts:
            try:
                ...
                if hasattr(self.scripts[script_name], "unload") and callable(self.scripts[script_name].unload):
                ...
                self.registered_commands = {
                    cmd: data for cmd, data in self.registered_commands.items()
                    if data['script_name'] != script_name
                }
                self.command_aliases = {
                    alias: target for alias, target in self.command_aliases.items()
                    if self.registered_commands.get(target, {}).get('script_name') != script_name
                }
                self.registered_help_texts = {
                     cmd: data for cmd, data in self.registered_help_texts.items()
                    if data['script_name'] != script_name
                }
                ...
                if hasattr(reloaded_module, "get_script_instance"):
                    ...
                    if script_instance:
                        ...
                        return True
                else: self.logger.warning(f"Reloaded script {script_name} has no 'get_script_instance' method")
            except Exception as e:
                self.logger.error(f"Failed to reload script {script_name}: {e}", exc_info=True)
                # If reload fails, try to restore the old instance if it was saved, or remove it
                if script_name in self.scripts: del self.scripts[script_name] # Remove broken script
        return False
    ...
    def get_all_script_commands_with_help(self) -> Dict[str, Dict[str, Any]]:
        ...
        This method now directly uses the help_text stored during command registration.
        """
        commands = {}
        for cmd_name, cmd_data in self.registered_commands.items():
            # cmd_data already contains 'help_text', 'aliases', 'script_name'
            # from register_command_from_script
            commands[cmd_name] = {
