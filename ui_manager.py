...
MIN_SIDEBAR_USER_LIST_WIDTH = 8 # Minimum practical width to attempt drawing user list items
...
class UIManager:
    ...
    def _draw_window_border_and_bkgd(self, window, color_attr, title=""):
        ...
        try:
            window.erase()
            window.bkgd(" ", color_attr) # This can also raise curses.error if window is invalid
            if title:
                ...
                if max_y > 0 and max_x > 0: # Ensure window has some dimension
                    if 1 < max_x -1: # Check if there's space for title and padding
                        ...
                        self._safe_addstr(window, 0, 1, text_to_render, curses.A_BOLD, "border_title")
        except curses.error as e:
            logger.warning(f"curses.error in _draw_window_border_and_bkgd for window {window!r}, title '{title[:30]}...': {e}")
        except Exception as ex:
            logger.error(f"Unexpected error in _draw_window_border_and_bkgd: {ex}", exc_info=True)


    def _safe_addstr(self, window: Any, y: int, x: int, text: str, attr: int, context_info: str = ""):
        """Safely adds a string to a curses window, with coordinate and boundary checks."""
        if not window:
            logger.debug(f"_safe_addstr ({context_info}): Attempted to draw on a non-existent window.")
            return
        ...
        try:
            ...
            logger.warning(f"_safe_addstr ({context_info}): curses.error getting getmaxyx for window {window!r}: {e}")
            return
        except Exception as ex: # Catch other unexpected errors
            logger.error(f"_safe_addstr ({context_info}): Unexpected error getting getmaxyx for window {window!r}: {ex}", exc_info=True)
            return
    ...
    def _safe_hline(self, window: Any, y: int, x: int, char: Any, n: int, attr: int, context_info: str = ""):
        ...
        if not window:
            logger.debug(f"_safe_hline ({context_info}): Attempted to draw on a non-existent window.")
            return

        try:
            max_y, max_x = window.getmaxyx()
        except curses.error as e:
            logger.warning(f"_safe_hline ({context_info}): curses.error getting getmaxyx for window {window!r}: {e}")
            return
        except Exception as ex:
            logger.error(f"_safe_hline ({context_info}): Unexpected error getting getmaxyx for window {window!r}: {ex}", exc_info=True)
            return
        ...
        try:
            ...
            logger.error(f"_safe_hline ({context_info}): Unexpected error drawing hline at y={y},x={x}: {ex}", exc_info=True)


    def _safe_move(self, window: Any, y: int, x: int, context_info: str = ""):
        """Safely moves the cursor in a window, with coordinate checks."""
        if not window:
            logger.debug(f"_safe_move ({context_info}): Attempted to move cursor in a non-existent window.")
            return

        try:
            max_y, max_x = window.getmaxyx()
        except curses.error as e:
            logger.warning(f"_safe_move ({context_info}): curses.error getting getmaxyx for window {window!r}: {e}")
            return
        except Exception as ex:
            logger.error(f"_safe_move ({context_info}): Unexpected error getting getmaxyx for window {window!r}: {ex}", exc_info=True)
            return
        ...
        try:
            ...
            logger.error(f"_safe_move ({context_info}): Unexpected error moving cursor to y={safe_y},x={safe_x}: {ex}", exc_info=True)
    ...
    def draw_messages(self, current_active_ctx_obj, current_active_ctx_name_str):
        ...
        if not active_ctx_obj:
            ...
            self._safe_addstr(self.msg_win, 0, 0, error_msg, self.colors.get("error", 0), "draw_messages_ctx_not_found")
            try:
                self.msg_win.noutrefresh()
            except curses.error as e:
                logger.warning(f"curses.error on noutrefresh in draw_messages (ctx not found): {e}")
            return
        ...
        for i, (msg_text, color_attr_val) in enumerate(lines_to_draw):
            ...
            self._safe_addstr(self.msg_win, draw_y, 0, msg_text, color_attr_val, "draw_messages_content")
    ...
    def _draw_sidebar_context_list(self, max_y: int, max_x: int, current_active_ctx_name_str: str) -> int:
        ...
        if not self.sidebar_win:
            return 0
        line_num = 0
        self._safe_addstr(self.sidebar_win, line_num, 0, "Windows:", self.colors.get("sidebar_header", 0), "_draw_sidebar_context_list_header")
        ...
        for ctx_name in all_contexts:
            ...
            self._safe_addstr(self.sidebar_win, line_num, 0, display_name_final, attr, "_draw_sidebar_context_list_item")
            line_num += 1

        # This gap increment seems to be for spacing, ensure it doesn't push line_num out of bounds for next header
        if line_num < max_y -1: # Check before incrementing for a potential hline or next header
             line_num += 1
        return line_num
    ...
    def _draw_sidebar_user_list_header(self, line_num: int, max_y: int, max_x: int, active_ctx_obj_for_users, current_active_ctx_name_for_user_header: str) -> int:
        ...
        if line_num > 0 and line_num < max_y: # Ensure hline is within bounds
            self._safe_hline(self.sidebar_win, line_num, 0, curses.ACS_HLINE, max_x, 0, "_draw_sidebar_user_list_hline")
        ...
        self._safe_addstr(self.sidebar_win, line_num, 0, user_header_full, self.colors.get("sidebar_header",0), "_draw_sidebar_user_list_header_text")
        line_num += 1
        return line_num
    ...
    def _draw_sidebar_user_list_items_and_indicators(self, line_num: int, max_y: int, max_x: int, active_ctx_obj_for_users) -> int:
        ...
        if max_x < MIN_SIDEBAR_USER_LIST_WIDTH:
            if line_num < max_y: # Ensure there's at least one line to draw the message
                self._safe_addstr(self.sidebar_win, line_num, 0, "[Users Hidden]",
        ...
        if up_indicator_text and line_num < max_y: # Check space for indicator
            self._safe_addstr(self.sidebar_win, line_num, 1, up_indicator_text, self.colors.get("sidebar_item", 0) | curses.A_DIM, "_draw_sidebar_user_list_up_indicator")
            line_num += 1
        ...
        for nick, prefix_str in visible_users_page:
            ...
            self._safe_addstr(self.sidebar_win, line_num, 0, " " + display_user_with_prefix, user_color, "_draw_sidebar_user_list_item")
            line_num += 1

        if down_indicator_text and line_num < max_y: # Check space for indicator
            self._safe_addstr(self.sidebar_win, line_num, 1, down_indicator_text, self.colors.get("sidebar_item", 0) | curses.A_DIM, "_draw_sidebar_user_list_down_indicator")
            line_num += 1
    ...
    def draw_sidebar(self, current_active_ctx_obj, current_active_ctx_name_str):
        ...
        try:
            ...
            logger.warning(f"curses.error getting getmaxyx for sidebar_win in draw_sidebar: {e}. Aborting draw.")
            return
        except Exception as ex:
            logger.error(f"Unexpected error getting getmaxyx for sidebar_win in draw_sidebar: {ex}", exc_info=True)
            return
    ...
    def draw_status_bar(self, current_active_ctx_obj, current_active_ctx_name_str):
        ...
        try:
            ...
            logger.warning(f"curses.error getting getmaxyx for status_win in draw_status_bar: {e}. Aborting draw.")
            return
        except Exception as ex:
            logger.error(f"Unexpected error getting getmaxyx for status_win in draw_status_bar: {ex}", exc_info=True)
            return
        ...
        try:
            ...
            logger.warning(f"curses.error re-getting getmaxyx for status_win in draw_status_bar: {e}. Aborting draw.")
            return
        except Exception as ex:
            logger.error(f"Unexpected error re-getting getmaxyx for status_win in draw_status_bar: {ex}", exc_info=True)
            return
        if max_y <= 0 or max_x <= 0: return # Check again
        ...
        self._safe_addstr(self.status_win, 0, 0, full_status_text, status_bar_color, "draw_status_bar_left")
        ...
        if len_status_left_drawn + len(status_right) + 1 < max_x : # +1 for a potential space
        ...
        elif len_status_left_drawn < max_x : # If there's any space left at all after left part
            ...
            if len(" | " + status_right) < remaining_space_for_right:
                 self._safe_addstr(self.status_win, 0, len_status_left_drawn, " | " + status_right, status_bar_color, "draw_status_bar_right_fallback")
    ...
    def draw_input_line(self):
        ...
        try:
            ...
            logger.warning(f"curses.error getting getmaxyx for input_win in draw_input_line: {e}. Aborting draw.")
            return
        except Exception as ex:
            logger.error(f"Unexpected error getting getmaxyx for input_win in draw_input_line: {ex}", exc_info=True)
            return
        ...
        try:
            ...
            logger.warning(f"curses.error re-getting getmaxyx for input_win in draw_input_line: {e}. Aborting draw.")
            return
        except Exception as ex:
            logger.error(f"Unexpected error re-getting getmaxyx for input_win in draw_input_line: {ex}", exc_info=True)
            return
        if max_y <= 0 or max_x <= 0: return # Check again
        ...
        cursor_pos_x = len(prompt) + len(display_buffer) # This is ideal cursor position

        input_color = self.colors.get("input", 0)
        self._safe_addstr(self.input_win, 0, 0, prompt + display_buffer, input_color, "draw_input_line_buffer")
        ...
        if resize_occurred:
            ...
            try:
                ...
                if self.height > 0 and self.width > 0:
                    ...
                    self.stdscr.clearok(True) # Force clear on next refresh after resize
                else:
                ...
                try:
                    ...
                    self.stdscr.clearok(True) # Ensure stdscr is also cleared properly on next refresh cycle
                    if self.msg_win:
                    ...
                    try:
                        ...
                        if self.height > 0 and self.width > 0:
                            ...
                            if msg_x + len(msg) <= self.width: # Check if message can fit
                                error_attr = self.colors.get("error", curses.A_BOLD | curses.color_pair(0))
                    ...
                    try:
                        ...
                        if self.height > 0 and self.width > 0:
                            ...
                            if err_x + len(generic_error_msg) <= self.width:
                                self.stdscr.addstr(err_y, err_x, generic_error_msg, self.colors.get("error", curses.color_pair(0)))
                        self.stdscr.refresh()
            ...
            try:
                ...
                if self.height > 0 and self.width > 0 :
                    ...
                    if err_x + len(draw_error_msg) <= self.width:
                        self.stdscr.addstr(err_y, err_x, draw_error_msg, self.colors.get("error", curses.color_pair(0)))
                self.stdscr.refresh()
            ...
            try:
                ...
                if self.height > 0 and self.width > 0:
                    ...
                    if err_x + len(critical_error_msg) <= self.width:
                        self.stdscr.addstr(err_y, err_x, critical_error_msg, self.colors.get("error", curses.color_pair(0)))
                self.stdscr.refresh()
