...
from pyrc_core.commands.dcc import dcc_commands
...
class IRCClient_Logic:
    ...
    def __init__(self, stdscr, args):
        ...
        self.network_handler = NetworkHandler(self)  # type: ignore[arg-type] # NetworkHandler uses client_logic_ref.channel_log_enabled
        ...
        if self.is_headless:
            ...
            self.input_handler = InputHandler(self)  # type: ignore[arg-type]
        ...
        self.script_manager = ScriptManager(
            self, BASE_DIR, disabled_scripts=cli_disabled.union(config_disabled)  # type: ignore[arg-type]
        )

        self.command_handler = CommandHandler(self)  # type: ignore[arg-type]
        self._initialize_connection_handlers()
        self.event_manager = EventManager(self, self.script_manager)  # type: ignore[arg-type]
    ...
    def _initialize_connection_handlers(self):
        ...
        self.cap_negotiator = CapNegotiator(
            ...
            client_logic_ref=self,  # type: ignore[arg-type]
        )
        sasl_pass = (
    ...
    def handle_server_message(self, line: str):
        ...
        irc_protocol.handle_server_message(self, line)  # type: ignore[arg-type]
    ...
    def _execute_python_trigger(
        ...
        try:
            python_trigger_api = PythonTriggerAPI(self)  # type: ignore[arg-type]
            execution_globals = {
                "__builtins__": {
    ...
    def initialize(self) -> bool:
        try:
            self.network_handler = NetworkHandler(self)  # type: ignore[arg-type]
            self.script_manager = ScriptManager(self, BASE_DIR)  # type: ignore[arg-type]
