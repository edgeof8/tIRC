...
class DummyUI:
    def __init__(self):
        self.colors = {
            "default": 0, "system": 0, "join_part": 0, "nick_change": 0,
            "my_message": 0, "other_message": 0, "highlight": 0, "error": 0,
            "status_bar": 0, "sidebar_header": 0, "sidebar_item": 0,
            "sidebar_user": 0, "input": 0, "pm": 0, "user_prefix": 0,
            "warning": 0, "info": 0, "debug": 0, "timestamp": 0,
            "nick": 0, "channel": 0, "query": 0, "status": 0,
            "list": 0, "list_selected": 0, "list_header": 0, "list_footer": 0,
            "list_highlight": 0, "list_selected_highlight": 0,
            "list_selected_header": 0, "list_selected_footer": 0,
            "list_selected_highlight_header": 0, "list_selected_highlight_footer": 0,
        }
...
class IRCClient_Logic:
    ...
    def __init__(self, stdscr, args):
        ...
        if hasattr(args, 'server') and args.server:
            ...
            temp_config = ServerConfig(
                server_id="CommandLine", address=args.server, port=cli_port, ssl=cli_ssl,
                nick=cli_nick, channels=cli_channels, server_password=cli_password,
                nickserv_password=cli_nickserv_password, verify_ssl_cert=app_config.VERIFY_SSL_CERT,
                auto_connect=True,
            )
            ...
            self.active_server_config = self.all_server_configs[app_config.DEFAULT_SERVER_CONFIG_NAME]
        ...
        if self.active_server_config:
            ...
            self.server = None; self.port = None
            self.initial_nick = app_config.DEFAULT_NICK; self.nick = app_config.DEFAULT_NICK
        ...
        if ENABLE_TRIGGER_SYSTEM:
            ...
            if not os.path.exists(config_dir_triggers):
                ...
                except OSError as e_mkdir: logger.error(f"Could not create config directory for triggers: {e_mkdir}")
        ...
        if platform.system() == "Windows": config_dir = os.path.join(os.getenv("APPDATA", os.path.expanduser("~")), "PyRC")
        elif platform.system() == "Darwin": config_dir = os.path.join(os.path.expanduser("~"), "Library", "Application Support", "PyRC")
        else: config_dir = os.path.join(os.path.expanduser("~"), ".config", "pyrc")
        ...
        if not self.active_server_config:
             self._add_status_message(
                "No server specified and no default configuration. Use /server <name> or /connect <host> to connect.", "warning",
            )
    ...
    def _initialize_connection_handlers(self):
        ...
        self.cap_negotiator = CapNegotiator(
            network_handler=self.network_handler, desired_caps=self.desired_caps_config,
            registration_handler=None, client_logic_ref=self,
        )
        ...
        self.sasl_authenticator = SaslAuthenticator(
            network_handler=self.network_handler, cap_negotiator=self.cap_negotiator,
            password=sasl_pass, client_logic_ref=self,
        )
        reg_initial_nick = self.nick
        reg_username = (self.active_server_config.username if self.active_server_config and self.active_server_config.username is not None else reg_initial_nick)
        reg_realname = (self.active_server_config.realname if self.active_server_config and self.active_server_config.realname is not None else reg_initial_nick)

        self.registration_handler = RegistrationHandler(
            network_handler=self.network_handler, command_handler=self.command_handler,
            initial_nick=reg_initial_nick, username=reg_username, realname=reg_realname,
            server_password=self.password, nickserv_password=self.nickserv_password,
            initial_channels_to_join=self.initial_channels_list[:],
            cap_negotiator=self.cap_negotiator, client_logic_ref=self,
        )
        self.cap_negotiator.registration_handler = self.registration_handler
        if hasattr(self.cap_negotiator, 'set_sasl_authenticator'): self.cap_negotiator.set_sasl_authenticator(self.sasl_authenticator)
        if hasattr(self.registration_handler, 'set_sasl_authenticator'): self.registration_handler.set_sasl_authenticator(self.sasl_authenticator)
        logger.debug("Connection handlers initialized.")

    def add_message(
        self, text: str, color_attr_or_key: Any, prefix_time: bool = True,
        context_name: Optional[str] = None, source_full_ident: Optional[str] = None,
        is_privmsg_or_notice: bool = False,
    ):
        resolved_color_attr: int
        if isinstance(color_attr_or_key, str):
            resolved_color_attr = self.ui.colors.get(color_attr_or_key, self.ui.colors.get("default", 0))
        ...
        target_context_name = context_name if context_name is not None else self.context_manager.active_context_name
        if not target_context_name: target_context_name = "Status"
        ...
        if not target_ctx_exists:
            ...
            if not self.context_manager.create_context(
                ...
                self.context_manager.add_message_to_context(
                    "Status", f"[CtxErr for {target_context_name}] {text}", resolved_color_attr,
                )
                self.ui_needs_update.set(); return

        target_context_obj = self.context_manager.get_context(target_context_name)
        if not target_context_obj: logger.critical(f"Context {target_context_name} unexpectedly None. Message lost: {text}"); return

        max_w = self.ui.msg_win_width - 1 if self.ui.msg_win_width > 1 else 80
        timestamp = time.strftime("%H:%M:%S ") if prefix_time else ""
        full_message = f"{timestamp}{text}" if not text.startswith(timestamp.strip()) else text

        lines = []; current_line = ""
        for word in full_message.split(" "):
            if current_line and (len(current_line) + len(word) + 1 > max_w): lines.append(current_line); current_line = word
            else: current_line += (" " if current_line else "") + word
        if current_line: lines.append(current_line)
        if not lines and full_message: lines.append(full_message)

        num_lines_added_for_this_message = len(lines)
        for line_part in lines:
            self.context_manager.add_message_to_context(target_context_name, line_part, resolved_color_attr, 1)

        if target_context_obj.type == "channel":
            channel_logger = self.get_channel_logger(target_context_name)
            if channel_logger: channel_logger.info(text)
        elif target_context_obj.name == "Status":
            if self.channel_log_enabled:
                status_logger = self.get_status_logger()
                if status_logger: status_logger.info(text)

        if target_context_name == self.context_manager.active_context_name and \
           hasattr(target_context_obj, "scrollback_offset") and target_context_obj.scrollback_offset > 0:
    ...
    def get_channel_logger(self, channel_name: str) -> Optional[logging.Logger]:
        if not self.channel_log_enabled: return None
        ...
        if logger_key in self.channel_loggers: return self.channel_loggers[logger_key]
        try:
            ...
            collided = False
            if os.path.normpath(channel_log_file_path) == os.path.normpath(main_app_log_file_path): collided = True
            elif os.path.normpath(channel_log_file_path) == os.path.normpath(status_window_log_file_path): collided = True
            if collided:
            ...
            if not os.path.exists(self.main_log_dir_path):
                try: os.makedirs(self.main_log_dir_path)
                except OSError as e: logger.error(f"Failed to create main log dir for {channel_name}: {e}"); return None

            file_handler = logging.handlers.RotatingFileHandler(
                channel_log_file_path, maxBytes=self.channel_log_max_bytes,
                backupCount=self.channel_log_backup_count, encoding="utf-8",
            )
        ...
        except Exception as e: logger.error(f"Failed to create logger for channel {channel_name}: {e}", exc_info=True); return None

    def get_status_logger(self) -> Optional[logging.Logger]:
        if not self.channel_log_enabled: return None
        if self.status_logger_instance: return self.status_logger_instance
        try:
            ...
            if not os.path.exists(self.main_log_dir_path):
                try: os.makedirs(self.main_log_dir_path, exist_ok=True)
                except OSError as e: logger.error(f"Failed to create main log dir for status log: {e}"); return None

            status_logger = logging.getLogger("pyrc.client_status")
            status_logger.setLevel(self.channel_log_level)
            file_handler = logging.handlers.RotatingFileHandler(
                status_log_file_path, maxBytes=self.channel_log_max_bytes,
                backupCount=self.channel_log_backup_count, encoding="utf-8",
            )
            file_handler.setFormatter(self.log_formatter)
        ...
        except Exception as e: logger.error(f"Failed to create logger for Status messages: {e}", exc_info=True); return None
    ...
    def switch_active_context(self, direction: str):
        ...
        if not context_names: return

        sorted_context_names = ["Status"] + sorted([name for name in context_names if name != "Status"], key=lambda x: x.lower()) \
                               if "Status" in context_names else sorted(context_names, key=lambda x: x.lower())

        current_active_name = self.context_manager.active_context_name
        if not current_active_name and sorted_context_names: current_active_name = sorted_context_names[0]
        elif not current_active_name: return

        try: current_idx = sorted_context_names.index(current_active_name)
        except ValueError: current_idx = 0; current_active_name = sorted_context_names[0] if sorted_context_names else ""
        if not current_active_name : return

        new_active_context_name = None
        if direction == "next": new_idx = (current_idx + 1) % len(sorted_context_names); new_active_context_name = sorted_context_names[new_idx]
        elif direction == "prev": new_idx = (current_idx - 1 + len(sorted_context_names)) % len(sorted_context_names); new_active_context_name = sorted_context_names[new_idx]
        else:
            if direction in sorted_context_names: new_active_context_name = direction
            else:
                try:
                    num_idx = int(direction) - 1
                    if 0 <= num_idx < len(sorted_context_names): new_active_context_name = sorted_context_names[num_idx]
                    else: self.add_message(f"Invalid window number: {direction}. Max: {len(sorted_context_names)}", "error", context_name=current_active_name); return
                except ValueError:
                    found_ctx = [name for name in sorted_context_names if direction.lower() in name.lower()]
                    if len(found_ctx) == 1: new_active_context_name = found_ctx[0]
                    elif len(found_ctx) > 1: self.add_message(f"Ambiguous window name '{direction}'. Matches: {', '.join(sorted(found_ctx))}", "error", context_name=current_active_name); return
                    else:
                        exact_match_case_insensitive = [name for name in sorted_context_names if direction.lower() == name.lower()]
                        if len(exact_match_case_insensitive) == 1: new_active_context_name = exact_match_case_insensitive[0]
                        else: self.add_message(f"Window '{direction}' not found.", "error", context_name=current_active_name); return
        ...
        if new_active_context_name:
            ...
            if target_ctx_to_activate and target_ctx_to_activate.type == "channel" and \
               target_ctx_to_activate.join_status and target_ctx_to_activate.join_status != ChannelJoinStatus.FULLY_JOINED:
                self.add_message(f"Channel {new_active_context_name} not fully joined (Status: {target_ctx_to_activate.join_status.name}).", "system", context_name=current_active_name)
            ...
            if self.context_manager.set_active_context(new_active_context_name):
                ...
                self.add_message(f"Error switching to window '{new_active_context_name}'.", "error", context_name=current_active_name)
    ...
    def switch_active_channel(self, direction: str):
        ...
        if not cyclable_contexts:
            self.add_message("No channels or Status window to switch to.", "system", context_name=self.context_manager.active_context_name or "Status")
            return

        current_active_name_str: Optional[str] = self.context_manager.active_context_name
        current_idx = -1
        if current_active_name_str:
            try: current_idx = cyclable_contexts.index(current_active_name_str)
        ...
        if new_active_channel_name_to_set:
            if self.context_manager.set_active_context(new_active_channel_name_to_set):
                ...
                self.add_message(f"Error switching to '{new_active_channel_name_to_set}'.", "error", context_name=current_active_name_str or "Status")

    def is_cap_negotiation_pending(self) -> bool: return self.cap_negotiator.is_cap_negotiation_pending()
    def is_sasl_completed(self) -> bool: return self.sasl_authenticator.is_completed() if self.sasl_authenticator else True
    def get_enabled_caps(self) -> Set[str]: return self.cap_negotiator.get_enabled_caps() if self.cap_negotiator else set()
    ...
    def handle_channel_fully_joined(self, channel_name: str):
        ...
        if self.last_join_command_target and self.context_manager._normalize_context_name(self.last_join_command_target) == normalized_channel_name:
            ...
            self.last_join_command_target = None; self.ui_needs_update.set()
        else:
            ...
            if (not active_ctx or active_ctx.name == "Status"):
                ...
                if normalized_channel_name in normalized_initial_channels:
                    ...
                    self.context_manager.set_active_context(normalized_channel_name); self.ui_needs_update.set()
    ...
    def _execute_python_trigger(self, code: str, event_data: Dict[str, Any], trigger_info_for_error: str):
        ...
        try:
            ...
            execution_globals = {"__builtins__": {
                ...
                "eval": eval, "str": str, "int": int, "float": float, "list": list, "dict": dict,
                "True": True, "False": False, "None": None, "len": len, "isinstance": isinstance,
                "hasattr": hasattr, "getattr": getattr, "setattr": setattr, "delattr": delattr, "Exception": Exception,
            }}
            execution_locals = {"client": self, "api": python_trigger_api, "event_data": event_data,
    ...
    def process_trigger_event(self, event_type: str, event_data: Dict[str, Any]) -> Optional[str]:
    ...
    def handle_text_input(self, text: str):
        ...
        if not active_ctx_name: self._add_status_message("No active window to send message to.", "error"); return
        active_ctx = self.context_manager.get_context(active_ctx_name)
        if not active_ctx: self._add_status_message(f"Error: Active context '{active_ctx_name}' not found.", "error"); return
        ...
        if active_ctx.type == "channel":
            if active_ctx.join_status == ChannelJoinStatus.FULLY_JOINED:
                ...
                if "echo-message" not in self.get_enabled_caps(): self.add_message(f"<{self.nick}> {text}", "my_message", context_name=active_ctx_name)
                elif self.echo_sent_to_status: self.add_message(f"To {active_ctx_name}: <{self.nick}> {text}", "my_message", context_name="Status")
            else: self.add_message(f"Cannot send message: Channel {active_ctx_name} not fully joined (Status: {active_ctx.join_status.name if active_ctx.join_status else 'N/A'}).", "error", context_name=active_ctx_name)
        elif active_ctx.type == "query":
            self.network_handler.send_raw(f"PRIVMSG {active_ctx_name} :{text}")
            if "echo-message" not in self.get_enabled_caps(): self.add_message(f"<{self.nick}> {text}", "my_message", context_name=active_ctx_name)
            elif self.echo_sent_to_status: self.add_message(f"To {active_ctx_name}: <{self.nick}> {text}", "my_message", context_name="Status")
        else: self._add_status_message(f"Cannot send messages to '{active_ctx_name}' (type: {active_ctx.type}). Try a command like /msg.", "error")
    ...
    def handle_rehash_config(self):
        ...
        try:
            ...
            if hasattr(self.context_manager, "max_history"): self.context_manager.max_history = app_config.MAX_HISTORY
            self._add_status_message("Configuration reloaded. Some changes (like main log file settings or server connection details if manually edited in INI) may require a /reconnect or client restart to fully apply.")
    ...
    def run_main_loop(self):
        ...
        if not self.is_headless: self.ui.refresh_all_windows()
        if not self.network_handler.network_thread or not self.network_handler.network_thread.is_alive():
            if self.server and self.port: self.network_handler.start()
            else:
                logger.warning("Network connection not started: server or port not configured.")
                if not self.active_server_config: self._add_status_message("Cannot connect: No server specified and no default configuration. Use /server <name> or /connect <host>.", "error")
        try:
            while not self.should_quit:
                if not self.is_headless and self.input_handler and self.ui:
                    key_code = self.ui.get_input_char()
                    if key_code != curses.ERR: self.input_handler.handle_key_press(key_code)
                    if self.ui_needs_update.is_set() or key_code != curses.ERR:
                        self.ui.refresh_all_windows()
                        if self.ui_needs_update.is_set(): self.ui_needs_update.clear()
                else:
                    if self.ui_needs_update.is_set():
                        if not self.is_headless and self.ui: self.ui.refresh_all_windows()
                        self.ui_needs_update.clear()
                time.sleep(0.05)
        except KeyboardInterrupt: logger.info("Keyboard interrupt received in main client loop. Initiating quit."); self.should_quit = True
            ...
            if not self.is_headless:
                ...
                try:
                    ...
                    pass
            else: logger.error(f"Curses-related error in headless main loop: {e}", exc_info=True)
            ...
            if hasattr(self, "script_manager") and self.script_manager:
                ...
                random_msg = self.script_manager.get_random_quit_message_from_scripts({"nick": temp_nick_for_quit, "server": temp_server_for_quit})
                if random_msg: quit_message_to_send = random_msg
            if hasattr(self, "event_manager") and self.event_manager:
                try: self.event_manager.dispatch_client_shutdown_final(raw_line="")
                except Exception as e_dispatch: logger.error(f"Error dispatching CLIENT_SHUTDOWN_FINAL: {e_dispatch}", exc_info=True)
            ...
            if self.network_handler:
                ...
                if self.network_handler.network_thread and self.network_handler.network_thread.is_alive():
                    self.network_handler.network_thread.join(timeout=2.0)
                    if self.network_handler.network_thread.is_alive(): logger.warning("Network thread did not join in time from IRCClient_Logic.")
                    else: logger.debug("Network thread joined successfully from IRCClient_Logic.")
    ...
    def initialize(self) -> bool:
        try:
            ...
            self.script_manager.load_scripts()
            if ENABLE_TRIGGER_SYSTEM: self.trigger_manager = TriggerManager(os.path.join(BASE_DIR, "config")); self.trigger_manager.load_triggers()
            else: self.trigger_manager = None
            return True
        except Exception as e: logger.error(f"Failed to initialize client: {str(e)}"); return False

    def connect(self, server: str, port: int, use_ssl: bool = False) -> bool:
        if not self.network_handler: logger.error("Network handler not initialized"); return False
        try: self.network_handler.update_connection_params(server=server, port=port, use_ssl=use_ssl); self.network_handler.start(); return True
        except Exception as e: logger.error(f"Failed to connect: {str(e)}"); return False

    def disconnect(self, quit_message: str = "Client disconnecting") -> None:
        if self.network_handler: self.network_handler.disconnect_gracefully(quit_message)
    ...
    def _reset_state_for_new_connection(self):
        ...
        if new_status_context:
            for msg_tuple in current_status_msgs: new_status_context.add_message(msg_tuple[0], msg_tuple[1])
            if hasattr(new_status_context, "scrollback_offset"): new_status_context.scrollback_offset = status_scroll_offset
        for ch_name in self.initial_channels_list:
            self.context_manager.create_context(
                ch_name, context_type="channel",
                initial_join_status_for_channel=ChannelJoinStatus.PENDING_INITIAL_JOIN,
            )
        if self.initial_channels_list: self.context_manager.set_active_context(self.initial_channels_list[0])
        else: self.context_manager.set_active_context("Status")
        self.currently_joined_channels.clear()
        self.last_join_command_target = None
        self.user_modes.clear()
        logger.info(f"Client state reset. Active context set to '{self.context_manager.active_context_name}'.")
        self.ui_needs_update.set()
